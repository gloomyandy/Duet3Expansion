...
#define SUPPORT_TMC2240_SPI		1			// drive the TMC2240 over SPI (selects the TMC51xx-style SPI driver)
...
constexpr uint32_t Tmc2240CurrentRange = 0x03;								// TMC2240 current range (3A)
constexpr uint32_t Tmc2240SlopeControl = 0x01;								// TMC2240 slope control (200V/us)
constexpr float Tmc2240Rref = 12.0;											// TMC2240 reference resistor in Kohms
constexpr float DriverFullScaleCurrent = 36000/Tmc2240Rref;					// in mA, with DRV_CONF range bits = 0x03
...
constexpr Pin GlobalTmcEnablePin = EnablePins[0];
constexpr Pin GlobalTmcCSPin	 = DriverSelectPins[0];
...
constexpr unsigned int Encoder_SpiChannel = 2;					// AS5047P on shared SPI channel 2 (PIO)
constexpr Pin EncoderCsPin    = GpioPin(19);					// GPIO19 EXT1 -> CS
#endif
...
constexpr unsigned int CANInstanceNumber = 0;		// SPICAN: not a RP CAN peripheral instance; value unused but required by CanInterface::Init
...
constexpr PinDescription PinTable[] =
{
	...
	{ PwmOutput::pwm0b,	AdcInput::none,		nullptr		},	// GPIO1  DRV_UART (not used; the TMC2240 runs in SPI mode)
